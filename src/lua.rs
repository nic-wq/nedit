...
pub fn run_script(script: &str, ctx: LuaContext, current_buffer_path: &Option<PathBuf>) -> Result<Vec<LuaAction>, String> {
    fn inner(script: &str, ctx: LuaContext, current_buffer_path: &Option<PathBuf>) -> mlua::Result<Vec<LuaAction>> {
        let lua = Lua::new();
        ...
        nedit.set("current_file", lua.create_function(move |_, ()| Ok(c_file.clone()))?)?;
        
        let c_content = ctx.current_content.clone();
        nedit.set("current_content", lua.create_function(move |_, ()| Ok(c_content.clone()))?)?;
        
        let c_sel = ctx.current_selection.clone();
        nedit.set("selection", lua.create_function(move |_, ()| Ok(c_sel.clone()))?)?;

        let c_dir = ctx.current_dir.clone();
        nedit.set("list_dir", lua.create_function(move |_, path: Option<String>| {
        ...
        nedit.set("read_file", lua.create_function(move |_, path: String| {
            let p = c_dir2.join(path);
            std::fs::read_to_string(p).or_else(|_| Ok("".to_string()))
        })?)?;

        let act1 = actions.clone();
        nedit.set("write_selection", lua.create_function(move |_, text: String| {
            act1.lock().unwrap().push(LuaAction::WriteSelection(text));
            Ok(())
        })?)?;

        let act2 = actions.clone();
        nedit.set("write_current_file", lua.create_function(move |_, text: String| {
        ...
        nedit.set("write_file", lua.create_function(move |_, (path, text): (String, String)| {
            act3.lock().unwrap().push(LuaAction::WriteFile(c_dir3.join(path), text));
            Ok(())
        })?)?;

        let c_dir4 = ctx.current_dir.clone();
        let act4 = actions.clone();
        nedit.set("create_file", lua.create_function(move |_, (path, text): (String, String)| {
            act4.lock().unwrap().push(LuaAction::CreateFile(c_dir4.join(path), text));
            Ok(())
        })?)?;

        let c_dir5 = ctx.current_dir.clone();
        let act5 = actions.clone();
        nedit.set("delete_file", lua.create_function(move |_, path: String| {
            act5.lock().unwrap().push(LuaAction::DeleteFile(c_dir5.join(path)));
            Ok(())
        })?)?;
            ...
            if is_script_file {
                for action in &final_actions {
                    let affects_self = match action {
                        LuaAction::WriteFile(p, _) | LuaAction::CreateFile(p, _) | LuaAction::DeleteFile(p) => {
                            p == script_path
                        }
                    ...
                    if affects_self {
                        return Err(mlua::Error::RuntimeError(
                            "Script cannot modify itself. Use Edit Lua Script in Command Palette to edit scripts.".to_string()
                        ));
                    }
                }
            }
    ...
    // Validate actions in live script mode
        ...
        for action in &actions {
            match action {
                LuaAction::WriteCurrentFile(_) => {
                    // In live script mode, WriteCurrentFile should only modify the current file
                    if ctx.current_file.is_empty() {
                        return Err("Error: WriteCurrentFile requires a valid target file in live script mode".to_string());
                    }
                }
                LuaAction::WriteFile(path, _) | LuaAction::CreateFile(path, _) | LuaAction::DeleteFile(path) => {
                    // In live script mode, can only modify the target file
